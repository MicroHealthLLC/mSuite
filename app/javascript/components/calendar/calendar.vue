...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      renderEvents(){
        ...
        this.fetchedEvents.forEach((currentValue, index, rEvents)=> {
          if (currentValue.is_disabled){
            ...
            this.eventNotifications(currentValue.startdate, currentValue.title)
          }
      ...
      async eventNotifications(timeOfEvent, title){
        const reg = await navigator.serviceWorker.getRegistration();
        Notification.requestPermission().then(permission => {
          if (permission !== 'granted') {
            alert('you need to allow push notifications');
          } else {
            const timestamp = new Date(timeOfEvent).getTime() - 15 * 60 * 1000;
