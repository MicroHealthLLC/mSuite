<template>
  <div>
    <div id="cal" class="row">
      ...
      <div
        class="col-3 text-info d-flex justify-content-end pr-5"
  ...
  methods: {
    ...
    createCalendar() {
      ...
      this.calendar.on("clickEvent", (eventObj) => {
        this.eventNode = this.currentMindMap.nodes.find(
      ...
      this.calendar.on("beforeUpdateEvent", (eventObj) => {
        ...
        let difference = this.getDateDifference(
          data.start,
          eventObj.changes.end.d.d
        );
        if (difference == 0) {
          if (
            eventObj.changes.end.d.d - data.start <= 0 &&
            data.isAllday == false
          ) {
    ...
    toggleCalendarView() {
      ...
      let calendarStore =
        this.calendar.store.getState().calendar.events.internalMap;
    ...
    moveCalendar(value) {
      ...
      if (this.currentMindMap.nodes && this.currentMindMap.nodes.length > 0)
    ...
    openRecurringEventsModal(data) {
      ...
      this.$refs["recurring-calendar-event-modal"].$refs[
        "RecurringCalendarEventModal"
      ].open();
    },
    ...
    async saveEvents(eventObj) {
      ...
      let data = {
        ...
        line_color: eventObj.raw.isSprint
          ? eventObj.backgroundColor
          : "#363636",
    ...
    deleteEvents() {
      ...
      if (this.showEvent) {
        ...
        if (selectedNode) {
          let childsParent = this.fetchedEvents.find(
            (n) => n.id == selectedNode.parent_node
          );
          if (childsParent) {
    ...
    async checkForChildren(parent, child) {
      if (parent.children.length == 1 && parent.children[0].id == child.id) {
        await http.patch(`/nodes/${parent.id}`, {
          is_sprint: false,
          line_color: "#363636",
        });
      }
        ...
        this.calendar.createEvents([
          {
            ...
            category:
              currentValue.is_sprint || currentValue.hide_children
                ? "allday"
                : "time",
            ...
            color: textColor,
            raw: {
              isSprint: currentValue.is_sprint,
              parentNode: currentValue.parent_node,
              standalone: currentValue.standalone,
            },
          },
        ]);
      });
    ...
    async eventNotifications(title) {
      ...
      Notification.requestPermission().then((permission) => {
        if (permission !== "granted") {
          ...
          reg.showNotification("mSuite Calendar", {
            tag: title,
            body: title,
            data: {
    ...
    closeModelPicker() {
      ...
      if (this.selectedEvent.style.backgroundColor) {
        this.selectedEvent.style.backgroundColor =
          this.showEvent.backgroundColor;
      }
    ...
    updateColorNode() {
      if (this.selectedEvent.children.length == 3)
        this.selectedEvent = this.selectedEvent.children[0];
        ...
        if (!eventObj.raw.isSprint) {
          ...
          nodeList.forEach((n) => {
            ...
            if (
              ...
              eventEnd <= nodeEnd.setSeconds(nodeEnd.getSeconds() + 1)
            ) {
              ...
              if (eventObj.raw.parentNode == null) {
                ...
                if (multiNodes.length == 1) {
                  ...
                  if (
                    ...
                    !multiNodes
                      .map((n) => n.id)
                      .includes(eventObj.raw.parentNode)
                  ) {
    ...
    updateEventColors(mindmap) {
      ...
      mindmap.nodes
        .filter((node) => !parentNodes.includes(node))
        .forEach((n) => {
      ...
      setTimeout(() => {
        longEvents.forEach((e) => {
          ...
          elements.forEach((event) => {
            ...
            if (
              child.querySelector(".toastui-calendar-weekday-resize-handle")
            ) {
              child.querySelector(
  ...
  mounted: async function () {
    ...
    setTimeout(() => {
      ...
      document.getElementById(
        "calendar"
      ).style.height = `calc(96vh - ${height}px)`;
    }, 100);
