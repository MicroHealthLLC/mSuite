    ...
    <div id="cal" class="row">
      <div class="col-9 d-flex px-5" @click="showEditEvent = false">
        <span class="py-2 pl-2">
          <a href="javascript:;" role="button" class="d-flex text-info pointer mr-3" @click.prevent="moveCalendar(-1)">
    ...
    <b-popover target="calendar" :show.sync="showEditEvent" class="editPopup" v-if="showEditEvent">
      <div class="w-100 mr-4">
        <div class="row d-flex flex-row-reverse my-3">
          <span @click="showEditEvent = false" role="button" class="material-icons col-1 px-0 text-danger">
            close</span>
          <span @click="editEventModal" role="button" class="material-icons col-2 pl-0 text-secondary">
            edit</span>
          <span @click="deleteEvents" role="button" class="material-icons col-2 pl-2">
            delete</span>
          <span v-if="showEvent.raw.isSprint || (!showEvent.raw.parentNode && !showEvent.raw.isSprint)"
            class="fas fa-eye-dropper color-picker mt-1 icon-opacity" @click="colorSelected = true">
          </span>
        </div>
        ...
        <div class="row my-2">{{ showEvent.raw.standalone ? 'Relationship: None' : showEvent.raw.parentNode &&
          currentMindMap.nodes.find(n => n.id === showEvent.raw.parentNode) ? 'Related to: ' + currentMindMap.nodes.find(n => n.id === showEvent.raw.parentNode).title : '' }}</div>
      </div>
    </b-popover>
    <add-calendar-event-modal :eventDates="eventDates" :showEvent="showEvent" :allEvents="allEvents"
      @createEvent="beforeEventCreate" @updateEvent="beforeEventUpdate" @openRecurringModal="openRecurringEventsModal"
      ref="add-calendar-event-modal">
    </add-calendar-event-modal>
    <recurring-calendar-event-modal :recurringEvents="recurringEvents" :recurringEventsDate="recurringEventsDate"
      @continue="getRecurringEventsDate" ref="recurring-calendar-event-modal">
    </recurring-calendar-event-modal>
    <div v-if="colorSelected">
      <div class="card card-position p-0 border-none mt-5">
        <color-palette :selected-node="eventNode" :uniqueColors="uniqueColors" @updateColorNode="updateColorNode"
