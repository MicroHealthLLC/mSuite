<template>
  <div v-if="!loading">
    <navigation-bar @exportToImage="exportImage" @resetMindmap="reset_stages" @undoMindmap="undoObj"
      @redoMindmap="redoObj" @sendLocals="sendLocals" :current-mind-map="currentMindMap" ref="kanbanNavigation"
      :defaultDeleteDays="defaultDeleteDays" :expDays="expDays" :deleteAfter="deleteAfter"
      :temporaryUser="temporaryUser" :isEditing="isEditing" :saveElement="saveElement" :userList="userList"
    ...
    <div class="row kanban_board" id="kanban-board">
      <kanban-board :stages="computedStages" :blocks="blocks" :config="config" @update-block="updateBlockPosition">
        <div v-for="stage, index in computedStages" :slot="stage" :key="index" class="w-100">
          <div class="w-100 mb-2 col-head" :id="'stage_'+index">
            <div class="d-inline-block w-100 block">
              <div class="pointer w-100 d-flex">
                <div @click="updateStage(stage)" class="w-75">
                  <textarea-autosize @keydown.enter.prevent.native :id="index" :rows="1" type="text"
                    v-debounce:3000ms="blurEvent" :value="stage" class="border-0 stage-title"
                    @blur.native="newStageTitle($event)" placeholder="Enter Stage Title" />
                </div>
                <div class="d-flex w-25 justify-content-end">
                  ...
                  <div class="pointer" @click="deleteStageConfirm(stage)">
                    <i class="fas ml-1 fa-times mt-1 icon-delete-stage" title="Delete Stage"></i>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="stage !== '' " @mouseover="hover_addtask = index" @mouseleave="hover_addtask = '' "
            :class="hover_addtask === index ? 'hover_task rounded' : ''" @click.prevent="addBlockToStage(stage)"
            class="pointer d-inline-block w-100" title="Add a Task">
            <a role="button" class="bg-transparent border-0 pe-none">
              <i class="fas fa-list position-absolute mt-1 text-dark"></i>
              <span class="task_plus ml-4 text-dark"> Add a Task </span>
            </a>
          </div>
        </div>
        <div v-for="block,index in blocks" :slot="block.id" :key="block.id">
          <div class="d-inline-block w-100 block">
            <div class="pointer w-100 d-flex" @click="selectedNode(index)">
              <textarea-autosize @keydown.enter.prevent.native :rows="1" type="text" v-debounce:3000ms="blurEvent"
                v-model="block.title" @blur.native="updateBlock(block, $event, index)"
                class=" border-0 resize-text block-title" placeholder="Add Title to Task" />
              <div class="pointer float-right">
                <div @click="deleteBlockConfirm(block)">
                  <i class="fas ml-3 fa-times text-danger position-relative icon-opacity" title="Delete Task"></i>
                </div>
                <div>
                  <i class="fas ml-3 fa-arrows-alt position-relative icon-opacity" title="Drag Task"></i>
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="colorSelected">
        <div class="card p-0 border-none color-picker-placement">
          <color-palette :treeNode="selectedStage" :nodes="allStages" :currentMindMap="currentMindMap"
            :customPallete="customPallete" :uniqueColors="uniqueColors" @updateColorNode="updateColor"
            @saveNodeColor="saveNodeColor" @closeModelPicker="closeModelPicker"
            @updateTreeChartNode="updateStageRequest"></color-palette>
        </div>
      </div>
    </div>
  </div>
