    ...
    methods: {
      updateVote(data){
        let mindmap = {
          ...
          user    : this.$store.state.getUser,
        }
        let id = this.currentMindMap.unique_key
        http.patch(`/msuite/${id}.json`,mindmap).then( res =>{
          this.pollEdit = false
          this.dataLoaded = true
          if(JSON.parse(res.data.mindmap.canvas).Questions[0].voters.length > 0) this.child_mindmap = true
        })
      },
      pollEditData() {
        this.pollEdit = true
      ...
      async getPollData(){
        ...
        if(response && response.data.mindmap){
          ...
          if(JSON.parse(response.data.mindmap.canvas).pollData.Questions[0].voters.length > 0) _this.child_mindmap = true
        }
      },
      exportXLS(option){
        if(this.pollData){
        ...
        if (this.pollData){
          if (this.pollData.Questions){
            let ques = this.pollData.Questions[0]
              if (ques.question != '' &&
                  ques.answerField &&
                  ques.answerField[0].text != '' &&
                  ques.answerField[1].text != '') this.dataLoaded = true
          }
