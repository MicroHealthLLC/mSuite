<template>
  <div>
    <b-list-group-item :id="'todo' + node.id" tabindex="0" @click="myTodo(node)" class="mb-0">
      <div class="flex" v-if="selectedTodo.id != node.id">
        <div class="col-8 d-flex flex-row">
          <div class="flex-auto">
            <input type="checkbox" class="mr-4" :checked="node.is_disabled"
            ...
            <b-form @submit.prevent="updateTodo(selectedTodo, selectedTodo.name, selectedTodo.is_disabled)">
              <b-row>
                ...
                <b-col sm="2" cols="2" class="d-flex flex-row">
                  <b-button v-b-tooltip.hover title="Save" type="submit" variant="success"> <i class="fas fa-save"></i>
                  </b-button>
                  <b-button class="ml-1" v-b-tooltip.hover title="Cancel" variant="secondary" @click="clearTodoEditObj">
                    <i class="fa fa-ban"></i></b-button>
                </b-col>
              </b-row>
            </b-form>
          </div>
    ...
    <div v-if="node.children && node.children.length">
      <draggable class="list-group" :list="sortedChildTodos" group="people" :move="handleMove"
        @change="(e) => handleEnd(e, sortedChildTodos)">
