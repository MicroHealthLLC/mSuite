...
export default {
  ...
  data: function () {
    ...
    return {
      ...
      isEditing: false,
      nodeSample: {
        label: "Enter node title for node",
        parent_label: "",
        color: "#CCBBBB",
      },
      customPallete: [],
        ...
        "*": (elementObject, value) => {
          elementObject.click(() => {
            if (event.target.tagName === "SPAN") {
              ...
              if (this.oldEventElement)
                this.oldEventElement.target.style.backgroundColor = "";
            }
          });
        },
      },
  ...
  methods: {
    onBindingComplete: function (event) {
      let nodestreeMaps = [];
      var nodeElement = this.insertNodeElement(
        "fas ml-2 fa-times text-danger cancel-btn mt-1 icon-opacity",
        "Delete Map"
      );
      var nodeElementSecond = this.insertNodeElement(
        "fas fa-plus add-icon cancel-btn mt-1 icon-opacity",
        "Add Child Node"
      );
      var nodeElementThird = this.insertNodeElement(
        "fas fa-eye-dropper text-dark cancel-btn mt-1 icon-opacity",
        "Color Picker"
      );
      ...
      event.target.children[0].append(
        nodeElement,
        nodeElementSecond,
        nodeElementThird
      );
    ...
    appendElementTreeMap(objArray) {
      ...
      objArray.forEach((e) => {
        ...
        var nodeElementFourth = this.insertNodeElement(
          "float-left fas fa-arrows-alt text-dark mt-1 icon-opacity",
          "Drag Node"
        );
        if (
          e.className == "jqx-treemap-rectangle jqx-treemap-rectangle-parent"
        ) {
    ...
    dropNode(e) {
      ...
      if (event.target.tagName === "DIV")
        dropNode.label = e.target.children[0].innerText;
    ...
    dragStart(e) {
      ...
      if (event.target.tagName === "DIV")
        dragNode.label = e.target.children[0].innerText;
    ...
    submitChildNode: async function (obj) {
      ...
      if (this.nodes.length > 0 && this.$store.state.nodeNumber != "NaN") {
        this.$store.dispatch(
          "setNodeNumber",
          parseInt(this.$store.state.nodeNumber) + 1
        );
      } else {
    ...
    setNodeSelected(value) {
      ...
      if (this.node.label && value.label != "Enter Node Title Here") {
        ...
        this.parent_node =
          this.node.label == this.currentMindMap.name ? true : null;
      } else if (value.parent.label && value.label == "Enter Node Title Here") {
        this.child_node = this.nodes.find(
          (n) => n.title === value.parent.label
        );
        this.parent_node =
          value.parent.label == this.currentMindMap.name ? true : null;
    ...
    textEdit(eventElement, value) {
      ...
      if (value.label == "Enter Node Title Here")
        return this.saveNodeElement(eventElement, value);
      var keyUpTimeOut;
      ...
      eventElement.target.addEventListener("blur", function () {
        if (_this.oldEventElement)
          _this.oldEventElement.target.style.backgroundColor = "";
      });
    ...
    saveNodeElement(eventElement, value) {
      ...
      eventElement.target.addEventListener(
        "keyup",
        () => {
          clearTimeout(keyUpTimeOut);
          _this.newNodeValue = document.getElementById("nodeField").value;
          if (event.keyCode === 13) {
            if (_this.newNodeValue && _this.newNodeValue != oldTitle)
    ...
    updateColorNode() {
      this.currentElementObj[0].style.backgroundColor =
        this.selectedNodeColor.line_color.hex;
    ...
    colorChange(value, elementObject) {
      ...
      this.oldElementColor = JSON.parse(
        JSON.stringify(this.currentElementObj[0].style.backgroundColor)
      );
