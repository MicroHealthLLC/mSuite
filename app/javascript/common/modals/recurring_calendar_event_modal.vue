  ...
  <sweet-modal ref="RecurringCalendarEventModal" class="of_v">
    ...
    <div class="w-100">
      <div class="row my-2">
        ...
        <span class="col-9 d-flex px-0">
          <input
            ...
            min="1"
            max="53"
            class="repeatNumber w-25 mx-2 form-control"
          />
          <select
            name="repeat"
            id="repeat"
      ...
      <div class="row my-2 mt-4">
        <span class="col-3 d-flex align-items-center pr-0"> Repeat on </span>
      ...
      <div class="row my-2 mt-4">
        <span
          class="col-12 form-label d-flex content-justified-start font-weight-bold"
        >
          Ends
        </span>
      </div>
      ...
      <div class="row my-2">
        ...
        <span
          class="datePicker col-6 mt-1 d-flex content-justified-start align-items-center ml-2"
        >
          <date-picker
            :disabled="endsOn != 'on'"
            v-model="endOnDate"
      ...
      <div class="row my-2">
        ...
        <span class="col-1 mt-2 px-0"> Occurences </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    calculateRecurrence() {
      if (!this.isValueInvalid) {
        ...
        let difference = this.getDateDifference(
          this.recurringEventsDate.start,
          this.recurringEventsDate.end
        );
        if (difference == 0) {
          let eventDate = new Date(this.recurringEventsDate.end);
          if (this.repeatType == "Month") {
            this.events = this.calculateMonthRecurrence(eventDate);
          } else if (this.repeatType == "Week") {
            this.events = this.calculateWeekRecurrence(eventDate);
          } else this.events = this.calculateDayRecurrence(eventDate);
        } else {
          ...
          if (this.repeatType == "Month") {
            ...
            let startDateEvents = this.calculateDayRecurrence(startDate);
            let endDateEvents = this.calculateDayRecurrence(endDate);
            this.mergeEvents(startDateEvents, endDateEvents);
          }
        }
    ...
    calculateMonthRecurrence(eventDate) {
      ...
      if (this.endsOn == "never") {
        while (12 - i >= this.repeatTime) {
          myDate = new Date(
            eventDate.setMonth(eventDate.getMonth() + parseInt(this.repeatTime))
          );
        ...
        if (this.endOnDate.getTime() > eventDate.getTime()) {
          while (12 - i >= this.repeatTime) {
            myDate = new Date(
              eventDate.setMonth(
                eventDate.getMonth() + parseInt(this.repeatTime)
              )
            );
    ...
    calculateWeekRecurrence(eventDate) {
      ...
      if (this.endsOn == "never") {
        while (52 - i >= this.repeatTime) {
          myDate = new Date(
            eventDate.setDate(
              eventDate.getDate() + parseInt(this.repeatTime) * 7
            )
          );
    ...
    calculateDayRecurrence(eventDate) {
      let count = 0;
      let newEvents = [];
      let myDate = null;
      if (this.endsOn == "after") {
        while (count < this.occurences) {
          myDate = new Date(
            eventDate.setDate(eventDate.getDate() + parseInt(this.repeatTime))
          );
          newEvents.push(myDate);
