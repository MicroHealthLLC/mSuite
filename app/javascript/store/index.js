...
const store = new Vuex.Store({
  ...
  actions: {
    createMsuite({ commit }, strMtype) {
      HTTP.post(`/msuite.json`, {
        mindmap: { name: "Central Idea", mm_type: strMtype },
      })
        .then((res) => {
          ...
          router.push({
            name: "maproot",
            params: { key: res.data.mindmap.unique_key },
          });
          router.go();
        })
        .catch((e) => {
    ...
    async updateMSuite({ commit, state }, obj) {
      await HTTP.patch(`/msuite/${state.mSuite.unique_key}`, obj).then(
        (res) => {
    ...
    async cloneMap({ state, dispatch }) {
      await HTTP.get(`/msuite/${state.mSuite.unique_key}/clone_map`).then(
        (res) => {
    ...
    async setCloneMapDate({ state, dispatch }, data) {
      await HTTP.post(`/msuite/${state.mSuite.unique_key}/clone_map`, {
        data: data,
      }).then((res) => {
        window.open(`/msuite/${res.data.mindmap.unique_key}`);
      });
    ...
    async resetMindmap({ commit, state }) {
      if (state.mSuite.mm_type != "kanban") {
        await HTTP.get(
          `/msuite/${state.mSuite.unique_key}/reset_mindmap.json`
        ).then((res) => {
          commit("setMSuite", res.data.mindmap);
        });
      } else {
        await HTTP.post("/stages/reset_stages", {
          mindmap_id: state.mSuite.id,
        });
      }
    },
    async undoMindmapNodes({ commit, state }, nodes) {
      await HTTP.post(`/msuite/${state.mSuite.unique_key}/undo_mindmap.json`, {
        undoNode: nodes,
      }).then((res) => {
    ...
    async redoMindmapNodes({ commit, state }, nodes) {
      await HTTP.put(`/msuite/${state.mSuite.unique_key}/redo_mindmap.json`, {
        redoNode: nodes,
      }).then((res) => {
